...
Функция ПолучитьТекстЗапросаОписанияПакетовНаСкладе(Ячейка, МассивПакетов = "") Экспорт 
	...
	Если ОбщегоНазначенияВызовСервераПовтИсп.БазаПУ().ЭтоERP Тогда
		...
		|	Вт.Ряд,
		|	Вт.Уровень,
		|	Вт.Номенклатура.pu_Порода.Наименование,
		|	pu_СведенияОПакетеСрезПоследних.Количество,
		|	pu_СведенияОПакетеСрезПоследних.КоличествоШтук,
		|	pu_СведенияОПакетеСрезПоследних.ОбъемСПрипуском,
		|	pu_СведенияОПакетеСрезПоследних.Сечение.Толщина,
		|	pu_СведенияОПакетеСрезПоследних.Сечение.Ширина,
		|	pu_СведенияОПакетеСрезПоследних.Длина.ДлинаОт,
		|	pu_СведенияОПакетеСрезПоследних.Длина.ДлинаДо,
		|	pu_СведенияОПакетеСрезПоследних.НомерПакета,
		|	pu_СведенияОПакетеСрезПоследних.ДатаВремяВыпуска,
		|	pu_СведенияОПакетеСрезПоследних.ПакетПиломатериалов.ПрефиксПроизводителя,
		|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(pu_СведенияОПакетеСрезПоследних.ПакетПиломатериалов),
		|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(pu_СведенияОПакетеСрезПоследних.ВидПродукции),
		|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(pu_СведенияОПакетеСрезПоследних.СортПиломатериалов),
		|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(pu_СведенияОПакетеСрезПоследних.ТипРаспила),
		|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(pu_СведенияОПакетеСрезПоследних.Влажность),
		|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(pu_СведенияОПакетеСрезПоследних.Партия),
		|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Вт.Ячейка),
		|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Вт.Склад)";
		//"ВЫБРАТЬ
		//|	ВложенныйЗапрос.Серия КАК Серия,
		//|	МАКСИМУМ(ВложенныйЗапрос.ВНаличииОстаток) КАК ВНаличииОстаток,
		//|	МАКСИМУМ(ТоварыНаСкладахОстатки.pu_ВНаличииШтукОстаток) КАК pu_ВНаличииШтукОстаток,
		//|	МАКСИМУМ(ТоварыНаСкладахОстатки.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)) КАК БезРезерва,
		...
		//|	
		//|	ИМЕЮЩИЕ
		//|		СУММА(ТоварыНаСкладахОстатки.ВНаличииОстаток) > 0) КАК ВложенныйЗапрос
		//|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыНаСкладах.Остатки(, УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Склад) В (&Склады)) КАК ТоварыНаСкладахОстатки
		//|		ПО (ВложенныйЗапрос.Серия = ТоварыНаСкладахОстатки.Серия)
		//|
		//|СГРУППИРОВАТЬ ПО
		...
		//|	СерииНоменклатурыДополнительныеРеквизиты.Свойство.Имя КАК СвойствоИмя,
		//|	СерииНоменклатурыДополнительныеРеквизиты.Значение КАК Значение
		//|ПОМЕСТИТЬ вт_СвойстваСерий
		//|ИЗ
		//|	вт_Остатки КАК вт_Остатки
		//|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СерииНоменклатуры.ДополнительныеРеквизиты КАК СерииНоменклатурыДополнительныеРеквизиты
		//|		ПО вт_Остатки.Серия = СерииНоменклатурыДополнительныеРеквизиты.Ссылка
		//|
		//|ИНДЕКСИРОВАТЬ ПО
		//|	Серия
		//|;
		...
		//|ВЫБРАТЬ РАЗЛИЧНЫЕ
		//|	ВложенныйЗапрос.Серия КАК Серия,
		//|	МАКСИМУМ(ВЫРАЗИТЬ(ВложенныйЗапрос.Пакет КАК Справочник.pu_ПакетыПиломатериалов)) КАК Пакет,
		//|	МАКСИМУМ(ВЫРАЗИТЬ(ВложенныйЗапрос.ТипРаспила КАК Справочник.pu_ТипыДосокПоРаспилу)) КАК ТипРаспила,
		//|	МАКСИМУМ(ВЫРАЗИТЬ(ВложенныйЗапрос.Длина КАК Справочник.pu_Длины)) КАК Длина
		//|ПОМЕСТИТЬ вт_Серии
		//|ИЗ
		//|	(ВЫБРАТЬ
		//|		вт_СвойстваСерий.Серия КАК Серия,
		//|		вт_СвойстваСерий.Значение КАК Пакет,
		//|		NULL КАК ТипРаспила,
		//|		NULL КАК Длина
		//|	ИЗ
		//|		вт_СвойстваСерий КАК вт_СвойстваСерий
		//|	ГДЕ
		//|		вт_СвойстваСерий.СвойствоИмя = ""ПакетСерии""
		//|	
		//|	ОБЪЕДИНИТЬ ВСЕ
		//|	
		//|	ВЫБРАТЬ
		//|		вт_СвойстваСерий.Серия,
		//|		NULL,
		//|		вт_СвойстваСерий.Значение,
		//|		NULL
		//|	ИЗ
		//|		вт_СвойстваСерий КАК вт_СвойстваСерий
		//|	ГДЕ
		//|		вт_СвойстваСерий.СвойствоИмя = ""ТипРаспилаСерии_ПМ""
		//|	
		//|	ОБЪЕДИНИТЬ ВСЕ
		//|	
		//|	ВЫБРАТЬ
		...
		//|		вт_СвойстваСерий КАК вт_СвойстваСерий
		//|	ГДЕ
		//|		вт_СвойстваСерий.СвойствоИмя = ""Длина_ПМ"") КАК ВложенныйЗапрос
		//|
		//|СГРУППИРОВАТЬ ПО
		//|	ВложенныйЗапрос.Серия
		//|
		...
		//|
		//|////////////////////////////////////////////////////////////////////////////////
		//|УНИЧТОЖИТЬ вт_СвойстваСерий
		//|;
		//|
		//|////////////////////////////////////////////////////////////////////////////////
		//|ВЫБРАТЬ
		//|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(ВложенныйЗапрос.Пакет) КАК НомерПакета,
		//|	pu_СведенияОПакетеСрезПоследних.Сечение.Толщина КАК Толщина,
		//|	pu_СведенияОПакетеСрезПоследних.Сечение.Ширина КАК Ширина,
		//|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(pu_СведенияОПакетеСрезПоследних.ВидПродукции) КАК ВидПродукции,
		//|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(pu_СведенияОПакетеСрезПоследних.СортПиломатериалов) КАК Сорт,
		//|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(pu_СведенияОПакетеСрезПоследних.ТипРаспила) КАК ТипРаспила,
		//|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(pu_СведенияОПакетеСрезПоследних.Влажность) КАК Влажность,
		//|	ВложенныйЗапрос.Длина.ДлинаОт КАК ДлинаОт,
		//|	ВложенныйЗапрос.Длина.ДлинаДо КАК ДлинаДо,
		//|	pu_СведенияОПакетеСрезПоследних.НомерПакета КАК НомерПакетаКод,
		//|	ВложенныйЗапрос.Пакет.ДатаВыпуска КАК НомерПакетаДатаВыпуска,
		//|	pu_СведенияОПакетеСрезПоследних.Номенклатура.pu_Порода.Наименование КАК Порода,
		//|	ВЫБОР
		//|		КОГДА ВложенныйЗапрос.БезРезерва
		//|			ТОГДА 1
		//|		ИНАЧЕ 2
		//|	КОНЕЦ КАК Состояние,
		//|	pu_СведенияОПакетеСрезПоследних.Количество КАК Объем,
		...
		//|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(pu_СведенияОПакетеСрезПоследних.Партия) КАК Партия,
		//|	pu_СведенияОПакетеСрезПоследних.ОбъемСПрипуском КАК ОбъемСПрипуском,
		//|	ВложенныйЗапрос.Пакет.ПрефиксПроизводителя КАК ПрефиксПроизводителя,
		//|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(РегАдреса.Ячейка) КАК Ячейка,
		//|	РегАдреса.Ряд КАК Ряд,
		//|	РегАдреса.Уровень КАК Уровень,
		//|	УНИКАЛЬНЫЙИДЕНТИФИКАТОР(РегАдреса.Склад) КАК Склад
		//|ИЗ
		//|	(ВЫБРАТЬ РАЗЛИЧНЫЕ
		//|		вт_Серии.Пакет КАК Пакет,
		//|		МАКСИМУМ(вт_Серии.Длина) КАК Длина,
		//|		МАКСИМУМ(вт_Серии.Серия) КАК Серия,
		//|		МАКСИМУМ(вт_Остатки.БезРезерва) КАК БезРезерва
		//|	ИЗ
		//|		вт_Серии КАК вт_Серии
		//|			ЛЕВОЕ СОЕДИНЕНИЕ вт_Остатки КАК вт_Остатки
		//|			ПО (вт_Остатки.Серия = вт_Остатки.Серия)
		//|	
		//|	СГРУППИРОВАТЬ ПО
		//|		вт_Серии.Пакет) КАК ВложенныйЗапрос
		//|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.pu_СведенияОПакете.СрезПоследних КАК pu_СведенияОПакетеСрезПоследних
		//|		ПО (ВложенныйЗапрос.Пакет = pu_СведенияОПакетеСрезПоследних.ПакетПиломатериалов)
		//|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.pu_ДвижениеПакетовПоЯчейкамСклада.СрезПоследних(, УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Склад) В (&Склады)) КАК РегАдреса
		//|		ПО (ВложенныйЗапрос.Серия = РегАдреса.СерияПакета)";
		...
		Если НЕ МассивПакетов = Неопределено Тогда
			ТекстОтборов = ТекстОтборов + Символы.ПС
			+ ?(ТекстОтборов="","","И ") + "УНИКАЛЬНЫЙИДЕНТИФИКАТОР(ВложенныйЗапрос.Пакет) В (&МассивГуидовПакетов)";
		КонецЕсли;
	Иначе
		ТекстЗапроса = 
		"ВЫБРАТЬ
...
Процедура ОчиститьРегистрОстатков(Ячейка = Неопределено, Знач ДанныеПУ = Неопределено, Склад = Неопределено)
	...
	Для Каждого склад из ОбщегоНазначенияВызовСервераПовтИсп.МассивСкладов() Цикл
		Если Ячейка = Неопределено Тогда
			...
			НаборЗаписей.Записать(ИСТИНА);  
		КонецЕсли;
		//	Если Ячейка = Неопределено и ДанныеПУ = Неопределено Тогда
	//		
	//		НаборЗаписей.Записать(ИСТИНА);
	//		
	//	ИначеЕсли  Ячейка = НЕОПРЕДЕЛЕНО Тогда 
	//		
	//		НаборЗаписей.Прочитать();
	//		МассивПакетовУдалить = НаборЗаписей.Выгрузить().ВыгрузитьКолонку("НомерПакета");
	//		МассивПакетовОставить = Новый Массив; 
	//		ДанныеСкладаПУ = ДанныеПУ.Скопировать(Новый Структура("Склад", склад.ИмяПредопределенногоПУ));
	//		МассивПакетовОставить = ДанныеСкладаПУ.ВыгрузитьКолонку("НомерПакета");
	//		МассивПакетовУдалить = ОбщегоНазначенияКлиентСервер.РазностьМассивов(МассивПакетовУдалить, МассивПакетовОставить);
	//		
	//		Для Каждого ИдПакета из МассивПакетовУдалить Цикл
	//			НачатьТранзакцию(РежимУправленияБлокировкойДанных.Управляемый);
	//			
	//			Блокировка = Новый БлокировкаДанных;
	//			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ОстаткиПакетовНаСкладах");
	//			ЭлементБлокировки.УстановитьЗначение("Склад", Склад);
	//			ЭлементБлокировки.УстановитьЗначение("НомерПакета", ИдПакета);
	...
	//			НЗ.Отбор.НомерПакета.Установить(ИдПакета);
	//			НЗ.Записать(ИСТИНА); 
	//			
	...
	Если Не СоотвТиповИзменений = Неопределено Тогда
		...
		Если ТипДанныхВыгрузки = "" или ТипДанныхВыгрузки = "ЗагрузитьИзмененияИзПУ" Тогда
			...
			Если ДанныеОтвета.Свойство("Изменения") Тогда
				СлужебныйОбновление.ЗагрузитьИзмененияИзПУ(ДанныеОтвета.Изменения, РезультатОбмена);
			КонецЕсли;
		...
		Если мОшибки.Количество()>0 Тогда
			ИсторияРаботы.мЗаписьИстории("СлужебныйОбновление.НачатьОбменИзменениями", СтрСоединить(мОшибки, Символы.ПС), ИСТИНА);
		Иначе
			ИсторияРаботы.мЗаписьИстории("СлужебныйОбновление.НачатьОбменИзменениями", "Длительность мс:"+(ТекущаяУниверсальнаяДатаВМиллисекундах() - НачалоОбмена), ЛОЖЬ)
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьСлужебныеСправочники(Фоново = ИСТИНА) Экспорт
	...
	Иначе
		...
		СлужебныйОбновление.ОбновитьВидыПоврежденияПакета();
		СлужебныйОбновление.ОбновитьВлажностиПМ();
		СлужебныйОбновление.ОбновитьТипыРаспила();
		СлужебныйОбновление.ОбновитьСортаПМ();
		СлужебныйОбновление.ОбновитьВидыПродукцииПМ();
		СлужебныйОбновление.ОбновитьЯчейкиСклада(); 
		СлужебныйОбновление.ОбновитьИнструкцииФотографирования();
