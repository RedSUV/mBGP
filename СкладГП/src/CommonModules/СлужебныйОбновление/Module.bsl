...
// Заполнить обновить преопределенный настройки.
...
Функция НайтиСоздатьСклад(База, ИмяПредопределенного, Наименование, ВспомогательныйСклад = "", ОтображатьПриАвторизации = ИСТИНА)
	...
	Если Выборка.Следующий() Тогда
		ОбъектСклад = Выборка.ПолучитьОбъект();
		ОбъектСклад.Владелец = База;
	Иначе
		ОбъектСклад = Справочники.Склады.СоздатьЭлемент();
		ОбъектСклад.Владелец = База;
	КонецЕсли;
	ОбъектСклад.ИмяПредопределенногоПУ = ИмяПредопределенного;
	ОбъектСклад.ВспомогательныйСклад = ВспомогательныйСклад;
	ОбъектСклад.ОтображатьПриАвторизации = ОтображатьПриАвторизации;
	ОбъектСклад.Наименование = Наименование;
	ОбъектСклад.ПометкаУдаления = ЛОЖЬ;
...
Функция СоздатьОбновитьЭлементЯчейки(СтрТаб, Менеджер) Экспорт
	...
	Если ЗначениеЗаполнено(Cтр.Родитель) Тогда
		...
		Если ЗначениеЗаполнено(Ячейка.Родитель) Тогда
			ПометкаУдаленияРодителя = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Ячейка.Родитель, "ПометкаУдаления");
			Если ПометкаУдаленияРодителя Тогда
				ОбъектРодителя = Ячейка.Родитель.ПолучитьОбъект();
				ОбъектРодителя.УстановитьПометкуУдаления(Ложь);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция СоздатьОбновитьЭлементИнструкцииФотографирования(Стр, Менеджер) Экспорт
	...
	Если ЗначениеЗаполнено(СпрСсылка.ВерсияДанных) Тогда
		ВерсияПУ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СпрСсылка, "ВерсияПУ");
		Если Строка(ВерсияПУ)  = стр.ВерсияДанных Тогда
			Возврат СпрСсылка;
		КонецЕсли;
		СпрОбъект = СпрСсылка.ПолучитьОбъект();
		СпрОбъект.ВерсияПУ = стр.ВерсияДанных;
...
Процедура ОчиститьРегистрОстатков(Ячейка = Неопределено, Знач ДанныеПУ = Неопределено, Склад = Неопределено)
	...
	НаборЗаписейОстаткиПакетов = РегистрыСведений.ОстаткиПакетовНаСкладах.СоздатьНаборЗаписей();
	НаборЗаписейОстаткиПакетов.ОбменДанными.Загрузка = ИСТИНА;
	
	//Очистим с пустым складом
	НаборЗаписейОстаткиПакетов.Отбор.Склад.Установить(Справочники.Склады.ПустаяСсылка());
	НаборЗаписейОстаткиПакетов.Записать(ИСТИНА);
	
	Для Каждого склад из ОбщегоНазначенияВызовСервераПовтИсп.МассивСкладов() Цикл
		Если Ячейка = Неопределено Тогда
			НаборЗаписейОстаткиПакетов.Отбор.Склад.Установить(Склад); 
			НаборЗаписейОстаткиПакетов.Записать(ИСТИНА);  
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Функция НачатьОбменИзменениями(КоличествоЗаписейВПакете = 100, ТипДанныхВыгрузки = "") Экспорт
	...
	Если ОбщегоНазначенияВызовСервераПовтИсп.ЭтоERP() Тогда
		ОписаниеИзменений = МобильноеПриложениеСкладПУ.ПолучитьИзмененияУзлаERP(УзелПУ);
	Иначе
		...
		Если Не СоотвТиповИзменений.Количество() = 0 Тогда
			...
			Если ДанныеДляПУ.Количество() > 0 Тогда
				...
				Если Ответ = Неопределено Тогда
					МассивОшибок.Добавить("Нет связи с сервером");
				ИначеЕсли Ответ.КодСостояния = 200 Тогда
					МобильноеПриложениеСкладПУ.ОчиститьИзмененияУзлаПУ();
				Иначе
					МассивОшибок.Добавить(Ответ.ПолучитьТелоКакСтроку());
				КонецЕсли;
			КонецЕсли;
		Иначе
			НомерОтправленного = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УзелПУ, "НомерОтправленного");
			МобильноеПриложениеСкладПУ.ОчиститьИзмененияУзлаПУ(НомерОтправленного);
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипДанныхВыгрузки = "" Или ТипДанныхВыгрузки = "ЗагрузитьИзмененияИзПУ" Тогда
		...
		Если Ответ = Неопределено Тогда
			МассивОшибок.Добавить("Ошибка связи");
	...
	РезультатОбмена.Вставить("Ошибки", МассивОшибок);
	Если МассивОшибок.Количество() > 0 Тогда
		ИсторияРаботы.мЗаписьИстории("СлужебныйОбновление.НачатьОбменИзменениями", СтрСоединить(МассивОшибок, Символы.ПС),
			Истина);
	Иначе
...
Процедура ОчиститьДанныеНаСервере(МетаданныеКУдалению, АдресРезультата = Неопределено) Экспорт
	...
	КонецЦикла;
	Для Каждого ПолноеИмяМетаданных из МетаданныеКУдалению.МассивСправочников Цикл
		МенеджерСправочника = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмяМетаданных);
