...
Функция ПривестиШаблон()
	...
	//Для инд = 1 по Ответ.Параметры.Количество() Цикл
	//	ЗначМассива = МассивПараметров[инд - 1];
	//	Ответ.Параметры.Вставить("Пар"+инд, ЗначМассива);
	//КонецЦикла;
...
Функция ВычислитьШаблон(ДанныеШаблона)
	...
	м = Запрос.Выполнить().Выбрать();
	м.Следующий();
	
	СтрПараметров = "м."+СтрСоединить(ДанныеШаблона.Параметры, ", м.");
