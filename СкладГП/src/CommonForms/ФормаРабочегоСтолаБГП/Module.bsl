...
Процедура ОбновитьЭлектроннуюОчередьСервер()
	...
	КонецЕсли;
	СтруктураЗапроса.ПараметрыЗапроса.Вставить("ТекущаяДата", ТекущаяДатаСеанса());
...
Процедура ИзменитьДанныеЗаписиРегистраСостоянияЗаявки(мЗаявка, ИмяКоманды)
	...
	Если ИмяКоманды  =	"ПогрузкаЗавершена" Тогда
		ДанныеСостояния.Вставить("Отгружено_Дата", ТекущаяДатаСеанса());
	ИначеЕсли ИмяКоманды  =	"ДокументыОформлены" Тогда
		ДанныеСостояния.Вставить("Оформлено_Дата", ТекущаяДатаСеанса());
	КонецЕсли;
	Успех = odata.ИзменитьЗаписьРегистраСостоянияЗаявки(мЗаявка, ДанныеСостояния);
	Если Не Успех Тогда
		Сообщить("Не удалось изменить состояние заявки");
	КонецЕсли;
