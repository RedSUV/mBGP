...
Процедура ЗаполнитьТаблицыФормы(ОчистилиВилы = ЛОЖЬ, Локально = ЛОЖЬ)
	ОшибкиОнлайн = ЛОЖЬ;
	ЯчейкиСклада.ЗаполнитьТаблицуРядыЯчейки(ЭтотОбъект, ВыбраннаяЯчейка, Локально, ОшибкиОнлайн);
	ЯчейкиСклада.ЗаполнитьТаблицуПакетовВил(ЭтотОбъект, ОчистилиВилы);
	Если ОшибкиОнлайн Тогда
		//@skip-check use-non-recommended-method
		Сообщить("Нет связи с сервером. Данные могут быть некорректные");
	КонецЕсли;
КонецПроцедуры
...
Процедура ДобавитьКомандуКонтекстногоМеню(ЭлементПакета, суфиксКоманды, Действие)
	...
	ЗаголовокКоманды = СуффиксыДобавленныхКоманд()[суфиксКоманды];
	...
	НовЭлемент.Заголовок = ЗаголовокКоманды;
	НовЭлемент.Высота = 2;	
...
Процедура СнятьВыделениеВыделенногоПакета()
	...
	ЭлементФормы = Элементы.Найти(э_ВыбранныйЭлементИмя); 
...
// Обработка выбора строки меню действий пакета.
// 
// Параметры:
//  ВыбранныйЭлемент - ЭлементФормы
//  Параметры - Структура Из Ключей:Элемент,СтрокаОписания
&НаКлиенте
	...
	Если ЗначениеЗаполнено(э_ВыбранныйЭлементИмя) и ЗначениеЗаполнено(э_ВыбранныйПакет) Тогда
		...
		Если ПакетВРядах Тогда
			...
			Иначе
				...
				ЯчейкиСкладаКлиент.РазместитьПакетВЯчейкеКлиент(э_ВыбранныйПакетID, ВыбраннаяЯчейка, СтрокаОписанияТекЯчейки.Ряд, СтрокаОписанияТекЯчейки.Уровень, ЛОЖЬ);	
				СнятьВыделениеВыделенногоПакета();
			КонецЕсли;
